...
恢复说明（2026-09-20）：按用户要求，Desktop 各产品入口恢复原始选择器，停止使用
将 Harness、来源、模型与思考档位混排的 A 版。新建任务显示独立引擎下拉；已有任务在
支持切换时先选引擎，再按供应商浏览模型；可保存引擎的设置入口同样先独立选引擎。
模型的思考档位与 Fast 沿用原始行级配置。复用当前模型目录和切换事务，不回退账号、
历史恢复与远程路由修复。以下 A 版面板说明保留为历史依据，不代表当前 Desktop 入口。
